...
import { getPwaInstallState, openPwaInstallDialog } from '../services/pwa-install-service.js';

const trashService = new TrashService(repositories, services.files);
...
async function saveGeneral(form){const data=new FormData(form);const changes={theme:value(data,'theme'),dateFormat:value(data,'dateFormat'),timeFormat:value(data,'timeFormat'),currency:value(data,'currency'),weekStartsOn:Number(value(data,'weekStartsOn',1))};await repositories.settings.save(changes);await refreshSettings();applyTheme(changes.theme);showToast('Instellingen opgeslagen.')}

const dailyCardChoices=[['appointments','Afspraken'],['departure','Vertrekken'],['packing','Meeneemlijsten'],['tasks','Taken'],['shopping','Boodschappen'],['birthdays','Verjaardagen'],['pets','Huisdieren'],['waste','Afval'],['inventory','Voorraad'],['notices','Prikbord'],['routines','Routines'],['subscriptions','Abonnementen'],['loans','Leenlijst']];
async function saveDailySettings(form){const data=new FormData(form);const dailyOverviewCards=data.getAll('dailyOverviewCards').map(String);if(!dailyOverviewCards.length)throw new Error('Kies minimaal één kaart voor het dagoverzicht.');await repositories.settings.save({dailyOverviewCards,trashRetentionDays:Number(value(data,'trashRetentionDays',0)),rewardsEnabled:data.has('rewardsEnabled')});await refreshSettings();showToast('Dagoverzicht, beloningen en prullenbak ingesteld.')}
...
function backgroundSyncText(status) {
  if (!status.eligible) return 'Achtergrondsynchronisatie wordt ingeschakeld zodra dit apparaat aan een gezinsaccount is gekoppeld.';
  if (status.periodicRegistered) return 'Achtergrondsynchronisatie is actief: wijzigingen krijgen een herhaalpoging en de browser mag de geïnstalleerde PWA periodiek bijwerken.';
  if (status.oneOffSupported) return 'Achtergrondsynchronisatie na wachtende wijzigingen is actief. Deze browser staat geen periodieke controles toe; openen, voorgrond en internetherstel blijven als terugval werken.';
  return 'Deze browser ondersteunt geen zelfstandige Background Sync. De app haalt automatisch in bij openen, voorgrond en internetherstel.';
}
...
async function openTrash() {
  ...
  const modal = openModal({
    ...
    content: items.length ? `<p class="muted">Herstel een item of verwijder de inhoud definitief. Definitief verwijderen kan niet ongedaan worden gemaakt.</p><ul class="item-list" id="trash-list">${items.map((item) => `<li class="list-item"><div class="list-item-main"><span class="badge">${e(item.type)}</span><strong class="list-item-title">${e(item.title)}</strong><span class="list-item-meta">Verwijderd op ${e(new Date(item.deletedAt).toLocaleString('nl-NL',{dateStyle:'medium',timeStyle:'short'}))}</span></div><div class="page-actions"><button class="button small secondary" data-trash-restore="${e(item.id)}" data-trash-entity="${e(item.entity)}">Herstellen</button><button class="button small danger" data-trash-purge="${e(item.id)}" data-trash-entity="${e(item.entity)}">Definitief</button></div></li>`).join('')}</ul>` : '<div class="empty-state"><strong>De prullenbak is leeg</strong>Er zijn geen verwijderde gegevens om te herstellen.</div>'
  });
  modal.querySelector('#trash-list')?.addEventListener('click', async (event) => {
    const purge = event.target.closest('[data-trash-purge]');
    if (purge) {
      if (!await confirmDialog({ title: 'Definitief verwijderen?', message: 'De inhoud en lokale versiegeschiedenis worden definitief verwijderd. Dit kan niet worden teruggedraaid.', confirmLabel: 'Definitief verwijderen' })) return;
      purge.disabled = true;
      try { await trashService.purge(purge.dataset.trashEntity, purge.dataset.trashPurge); purge.closest('li').remove(); showToast('Item definitief verwijderd.'); await updateTrashCount(); } catch (error) { purge.disabled = false; handleError(error); }
      return;
    }
...
export const settingsView={
  async render(){const settings=appState.settings;const [deletedItems,pushStatus,backgroundStatus]=await Promise.all([trashService.getDeletedItems(),services.push.status(),services.backgroundSync.status()]);const installState=getPwaInstallState();const appInstalled=installState.standalone||installState.installed;const categoryLabels={appointments:'Agenda',shopping:'Boodschappen',expenses:'Uitgaven',outings:'Uitjes'};const notificationLabel=pushStatus.subscribed?'Pushmeldingen uitschakelen':appState.cloud.family?'Pushmeldingen inschakelen':globalThis.Notification?.permission==='granted'?'Lokale meldingen zijn toegestaan':'Lokale meldingen toestaan';return `<section class="page-stack">
    ...
    <section class="card"><h2>Ontwikkelaarsinformatie</h2><dl class="small"><dt>Appversie</dt><dd>${APP_VERSION}</dd><dt>Databaseversie</dt><dd>${DATABASE_VERSION}</dd><dt>Lokale opslag</dt><dd>IndexedDB, offline-first</dd><dt>Centrale dienst</dt><dd>Supabase met Row Level Security</dd><dt>Synchronisatie</dt><dd>Live Supabase-updates op actieve apparaten, outboxgestuurd na iedere wijziging en PWA-achtergrondtaken waar ondersteund.</dd></dl></section>
  </section>`},
  async mount(root){
    ...
    const installButton=root.querySelector('#install-app');installButton.addEventListener('click',()=>openPwaInstallDialog());window.addEventListener('samen-thuis-install-state',()=>{const state=getPwaInstallState();const installed=state.standalone||state.installed;installButton.disabled=installed;installButton.textContent=installed?'App is geïnstalleerd':'App installeren'},{once:true})
  }
};
