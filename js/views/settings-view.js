...
export const settingsView={
  async render(){const settings=appState.settings;const [deletedItems,pushStatus,backgroundStatus]=await Promise.all([trashService.getDeletedItems(),services.push.status(),services.backgroundSync.status()]);const categoryLabels={appointments:'Agenda',shopping:'Boodschappen',expenses:'Uitgaven',outings:'Uitjes'};const notificationLabel=pushStatus.subscribed?'Pushmeldingen uitschakelen':appState.cloud.family?'Pushmeldingen inschakelen':globalThis.Notification?.permission==='granted'?'Lokale meldingen zijn toegestaan':'Lokale meldingen toestaan';return `<section class="page-stack">
