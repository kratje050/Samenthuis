importScripts('./js/sync/entity-catalog.js', './js/workers/background-sync-worker.js');

const CACHE_VERSION = 'samen-thuis-v3.1.0-r1';
const APP_SHELL = [
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clients) => {
    const target = event.notification.data?.url || event.notification.data?.route || './#agenda';
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(Promise.all([
    self.registration.showNotification(payload.title, {
      ...
      vibrate: [180, 80, 180], silent: false,
      actions: [{ action: 'open', title: 'Open Samen Thuis' }, { action: 'dismiss', title: 'Sluiten' }],
      data: { url: payload.url || './#agenda', kind: payload.kind || 'reminder' }
    }),
